...
from app.keyboards.inline import profile_confirm_keyboard, profile_gender_keyboard, profile_interest_keyboard
...
from app.utils.validators import parse_interests, validate_age, validate_bio, validate_nickname
...
async def _send_prompt(message: Message, state: FSMContext, text: str, reply_markup=None) -> None:
...
async def begin_profile_setup(message: Message, app: AppContext, user: dict[str, Any], state: FSMContext, restart: bool = False) -> None:
    ...
    await _send_prompt(message, state, app.i18n.t(user.get("language"), "ask_nickname"))
...
async def command_profile(message: Message, app: AppContext, db_user: dict[str, Any]) -> None:
    ...
    if not db_user.get("is_profile_complete"):
        ...
        return
    if db_user.get("profile_photo_file_id"):
        await message.answer_photo(db_user["profile_photo_file_id"], caption=profile_card(db_user))
    else:
...
async def profile_interest(callback: CallbackQuery, app: AppContext, db_user: dict[str, Any], state: FSMContext) -> None:
    ...
    if callback.message:
        await _send_prompt(callback.message, state, app.i18n.t(db_user.get("language"), "ask_bio"))
...
async def profile_photo_invalid(message: Message, app: AppContext, db_user: dict[str, Any], state: FSMContext) -> None:
    await _consume_answer(message, state)
...
async def profile_location(message: Message, app: AppContext, db_user: dict[str, Any], state: FSMContext) -> None:
    await _consume_answer(message, state)
...
async def profile_location_invalid(message: Message, app: AppContext, db_user: dict[str, Any], state: FSMContext) -> None:
    await _consume_answer(message, state)
    await _send_prompt(
        message,
        state,
...
async def show_profile_confirmation(message: Message, app: AppContext, db_user: dict[str, Any], state: FSMContext) -> None:
    ...
    await _delete_prompt_if_exists(message, state)
    await message.answer(app.i18n.t(db_user.get("language"), "confirm_profile_text"), reply_markup=remove_reply_keyboard())
