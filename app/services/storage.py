...
class TelegramChannelStorage:
    ...
    async def connect(self) -> None:
        ...
        logger.info("Telethon storage client connected.")
    ...
    async def rebuild_cache(self) -> None:
        ...
        count = 0
        async for message in self.client.iter_messages(self.settings.private_db_channel_id, reverse=True):
    ...
    async def append_record(self, record: BaseModel) -> int:
        async with self._lock:
            ...
            message = await self.client.send_message(self.settings.private_db_channel_id, body)
            env = StoredEnvelope(
                version=version,
    ...
    def get_user_profile(self, user_id: int) -> Optional[UserProfileRecord]:
        ...
        if profile.premium_until and profile.premium_until <= datetime.now(timezone.utc):
            # expire lazily in-memory; persistent update can happen on next write
