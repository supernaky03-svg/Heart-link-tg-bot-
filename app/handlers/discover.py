...
def _discover_variants() -> set[str]:
    return {t("en", "menu_discover"), t("my", "menu_discover"), t("ru", "menu_discover")}
...
async def show_next_profile(message: Message, app: AppContext) -> None:
    ...
    if not viewer or not viewer.is_active or viewer.is_banned or (settings and (settings.deleted or settings.paused)):
...
@router.message(lambda m: (m.text or "").strip() in _discover_variants())
