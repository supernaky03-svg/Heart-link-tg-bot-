...
def _profile_variants() -> set[str]:
    return {t("en", "menu_profile"), t("my", "menu_profile"), t("ru", "menu_profile")}
...
async def show_profile(message: Message, app: AppContext) -> None:
    ...
    if not profile or not profile.is_active or (settings and settings.deleted):
...
async def profile_command(message: Message, app: AppContext) -> None:
    await show_profile(message, app)


@router.message(lambda m: (m.text or "").strip() in _profile_variants())
...
async def edit_value(message: Message, state: FSMContext, app: AppContext) -> None:
    ...
    if field == "age":
        ...
        if age < app.settings.min_age:
            ...
            return
        await app.storage.update_user_profile(message.from_user.id, age=age)
    elif field == "gender":
        gender = parse_gender(value)
        if not gender:
            await message.answer(t(lang, "ask_gender"), reply_markup=gender_keyboard(lang))
            return
        await app.storage.update_user_profile(message.from_user.id, gender=gender)
    elif field == "looking_for":
        looking_for = parse_looking_for(value)
        if not looking_for:
            await message.answer(t(lang, "ask_looking_for"), reply_markup=looking_for_keyboard(lang))
            return
        await app.storage.update_user_profile(message.from_user.id, looking_for=looking_for)
    elif field == "city":
        city = sanitize_text(value)
        if not city:
            await message.answer(t(lang, "ask_city"))
            return
        await app.storage.update_user_profile(message.from_user.id, city=city)
    elif field == "name":
        name = sanitize_text(value)
        if not name or len(name) > app.settings.max_name_length:
            await message.answer(t(lang, "name_invalid"))
            return
        await app.storage.update_user_profile(message.from_user.id, name=name)
    elif field == "bio":
        bio = sanitize_text(value)
        if not bio or len(bio) > app.settings.max_bio_length:
            await message.answer(t(lang, "bio_invalid"))
            return
        await app.storage.update_user_profile(message.from_user.id, bio=bio)
    await state.clear()
    await message.answer(t(lang, "profile_updated"), reply_markup=main_menu_keyboard(lang))
...
async def edit_location(message: Message, state: FSMContext, app: AppContext) -> None:
    ...
    await state.clear()
    await message.answer(t(lang, "location_saved"), reply_markup=main_menu_keyboard(lang))
...
async def edit_media_finish(message: Message, state: FSMContext, app: AppContext) -> None:
    lang = get_user_language(app.storage, message.from_user.id)
    data = await state.get_data()
    ...
    if text == t(lang, "btn_done_save"):
        ...
        if not media_file_ids or not media_type:
            await message.answer(t(lang, "media_min_required"))
            return
        await app.storage.update_user_profile(message.from_user.id, media_type=media_type, media_file_ids=media_file_ids)
        await state.clear()
        await message.answer(t(lang, "profile_updated"), reply_markup=main_menu_keyboard(lang))
        return
