...
def _menu_variants(key: str) -> set[str]:
    return {t("en", key), t("my", key), t("ru", key)}
...
async def start_command(message: Message, state: FSMContext, app: AppContext) -> None:
    ...
    if profile and profile.is_active and not profile.is_banned and not settings.deleted and not settings.paused:
...
@router.message(lambda m: (m.text or "").strip() in _menu_variants("menu_language") | {t("en", "btn_change_language"), t("my", "btn_change_language"), t("ru", "btn_change_language")})
...
@router.message(lambda m: (m.text or "").strip() in _menu_variants("menu_help"))
